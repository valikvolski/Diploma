/**
 * Admin doctor edit: auto-upload avatar (POST /admin/doctors/:id/avatar as JSON).
 */
(function () {
  'use strict';
  ...
  document.addEventListener('DOMContentLoaded', function () {
    var root = document.querySelector('[data-admin-doctor-avatar-upload]');
    if (!root) return;
    var uploadUrl = root.getAttribute('data-upload-url') || '';
    if (!uploadUrl) return;

    var input = document.getElementById('adminDoctorAvatarFile');
    var holderWrap = document.getElementById('adminDoctorAvatarHolder');
    var holder = holderWrap ? holderWrap.querySelector('.user-avatar') : null;
      ...
      fetch(uploadUrl, {
        ...
        .then(function (out) {
          ...
          if (out.ok && out.data && out.data.ok && out.data.avatarUrl) {
            ...
            if (window.showAppToast) {
              window.showAppToast(out.data.message || 'Фото врача обновлено', 'success');
            }
            return;
          }
          var err =
